    ...
    return this.each(function() {
      ...
      finished = function( inputdata ) {
        ...
        var items = data.items,
            ...
            // items
            ul = $('<script>$(\'ul.lifestream li:first-child\').addClass( \'first-item\' );</script><ul class="' + settings.classname + '"/>');
        ...
        for ( ; i < length; i++ ) {
          ...
          if ( item.html ) {
            $('<li class="'+ settings.classname + '-'
              + item.config.service + '"><div class="spacer-left"></div><div class="spacer-center"></div><div class="spacer-right"></div><div class="spacer-tail"></div>').data( "name", item.config.service )
                                           .data( "url", item.url || "#" )
                                           .data( "time", item.date )
                                           .append( item.html )
                                           .appendTo( ul );
...
$.fn.lifestream.feeds.bitly = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      created: '<div class="bitly-box bitly-link"></div>Shared Something Cool <a href="${short_url}">'
        + '${short_url}</a>'
    },
    config.template);
...
$.fn.lifestream.feeds.delicious = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      bookmarked: '<div class="delicious-box delicious-link"></div>Saved A new bookmark <a href="${u}">${d}</a>'
    },
    config.template);
...
$.fn.lifestream.feeds.dribbble = function( config, callback ) {
    ...
    var template = $.extend({},
      {
        posted: '<div class="dribbble-box dribbble-link"></div>Posted a New Shot <a href="${url}">${title}</a>'
      },
      config.template);
...
$.fn.lifestream.feeds.forrst = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      posted: '<div class="forrst-box forrst-link"></div>posted a new ${post_type} '
...
$.fn.lifestream.feeds.github = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      pushed: '<div class="github-box github-link"></div><a href="${status.url}" title="{{if title}}${title} '
        +'by ${author} {{/if}}">pushed</a>&nbsp;&nbsp; to <a href="http://github.com/'
        +'${repo}/tree/${branchname}">${branchname}</a>&nbsp;&nbsp; at '
        +'<a href="http://github.com/${repo}">${repo}</a>',
      gist: '<div class="github-box github-link"></div><a href="${status.payload.url}" title="'
        +'${status.payload.desc || ""}">${status.payload.name}</a>',
      commented: '<div class="github-box github-link"></div>commented on <a href="${status.url}">${what}</a>&nbsp;&nbsp; on '
        +'<a href="http://github.com/${repo}">${repo}</a>',
      pullrequest: '$<div class="github-box github-link"></div>{status.payload.action} <a href="${status.url}">'
        +'pull request #${status.payload.number}</a> on '
        +'<a href="http://github.com/${repo}">${repo}</a>',
      created: '<div class="github-box github-link"></div>created ${status.payload.ref_type || status.payload.object}'
        +' <a href="${status.url}">${status.payload.ref || '
        +'status.payload.object_name}</a>&nbsp;&nbsp; for '
        +'<a href="http://github.com/${repo}">${repo}</a>',
      createdglobal: '<div class="github-box github-link"></div>created ${status.payload.object} '
        +'<a href="${status.url}">${title}</a>',
      deleted: '<div class="github-box github-link"></div>deleted ${status.payload.ref_type} ${status.payload.ref} '
...
$.fn.lifestream.feeds.googleplus = function( config, callback ) {
  ...
  var template = $.extend({},
    {
    posted: '<div class="googleplus-box googleplus-link"></div><!--<a href="${actor.url}">${actor.displayName}</a>has posted a new entry-->Posted a new entry <a href="${url}" '
        + '>${title}</a> <!--With--> <!--${object.replies.totalItems} replies, ${object.plusoners.totalItems} +1s, ${object.resharers.totalItems} Reshares-->'
    },
    config.template),
...
$.fn.lifestream.feeds.tumblr = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      posted: '<div class="tumblr-box tumblr-link"></div>posted a new ${type} <a href="${url}" >${title}</a>'
    },
    config.template),
...
$.fn.lifestream.feeds.twitter = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      posted: '<div class="twitter-box twitter-link"></div>{{html tweet}}'
    },
    config.template),
...
$.fn.lifestream.feeds.vimeo = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      posted: '<div class="vimeo-box vimeo-link"></div>Posted a video <a href="${url}">${title}</a>'    
    },
    config.template),
...
$.fn.lifestream.feeds.youtube = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      favorited: '<div class="youtube-box youtube-link"></div>Favorited a video <a href="${video.player.default}" '
